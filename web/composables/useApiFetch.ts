...
function onRequest({ request, options }: any) {
	...
	if (!options.headers['Authorization'] && tokens.value && tokens.value.accessToken) {
...
async function onResponse(context: any): Promise<void> {
	...
	if (isRefreshing) return new Promise(resolve => failedQueue.push({ resolve, failedConfig: context }));
	...
	if (!data || !data.value) {
		for (const { resolve, failedConfig } of failedQueue) resolve(failedConfig);
		isRefreshing = false;
		failedQueue = [];
	...
	setTokenStates({ accessToken: newTokens.access_token, refreshToken: newTokens.refresh_token });
	...
	const [current] = await Promise.all([
		...
		...failedQueue.map(async ({ resolve, failedConfig }) => {
			...
			const failed = await useFetch(failedConfig.request, failedConfig.options);
	...
	const { fetchHotels, clearHotels } = useHotelsStore();
	if (current.error && current.error.value) return clearHotels();
