...
export async function getCryptoById(_id: ObjectId) {
    const doc = await db
        .collection(collection)
        .findOne<CryptoCurrency>({ _id });

    return doc
        ? doc
        : Promise.reject({ status: 404, error: 'cryptocurrency not found' });
}

export async function getCryptoByName(name: string) {
    const doc = await db
        .collection(collection)
        .findOne<CryptoCurrency>({ name });

    if (!doc)
        return Promise.reject({
            status: 400,
            error: `cryptocurrency with name: ${name} not found`
        });
...
export async function getCryptoByUUID(uuid: string) {
    const doc = await db
        .collection(collection)
        .findOne<CryptoCurrency>({ uuid });

    if (!doc)
        return Promise.reject({
            status: 400,
            error: `cryptocurrency with uuid: ${uuid} not found`
        });
...
export async function createCrypto(newCrypto: CryptoCurrency) {
    ...
    if (cryptoAlreadyExists) {
        return Promise.reject({
            status: 409,
            error: `crypto ${newCrypto.name} already exists.`
        });
    }
...
export async function updateCryptoByUUID(
    uuid: string,
    crypto: Partial<CryptoCurrency>
) {
    if (!uuid || uuid.length == 0)
        return Promise.reject({ status: 404, error: 'crypto not found' });
    if (!crypto)
        return Promise.reject({
            status: 400,
            error: 'missing new crypto data'
        });
    ...
    return doc
        ? (doc as CryptoCurrency)
        : Promise.reject({ status: 404, error: 'crypto not found' });
}
